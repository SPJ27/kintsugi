...
export default function SideBar({
  ...
  return (
    <aside
      className={`hidden group select-none z-20 md:flex h-screen flex-col items-center justify-between gap-4 border-r-2 border-dashed border-[#c9a030] bg-[#2A1A08] py-4 text-center transition-[width] duration-500 ease-out ${kalam.className} ${pinned ? "sticky top-0 w-60" : "sticky top-0 w-20 hover:w-60  absolute top-0 left-0"}`}
    >
      <div className="flex flex-col gap-12 w-full">
        <div
          className={`w-full delay-200 duration-300 transition-all items-center text-center ${pinned ? "flex justify-between px-3" : "group-hover:flex group-hover:justify-between group-hover:px-3"}`}
        >
          <div
            className={`text-[#AF8937] ${pinned ? "text-xl" : "group-hover:text-xl"} font-bold`}
          >
            金継ぎ
          </div>
          <button
          ...
          >
            <Pin
              size={16}
              className={`${pinned ? "rotate-45 text-[#AF8937]" : ""}`}
            />
          </button>
        </div>
        <div className="flex gap-5 flex-col items-center text-center justify-center">
          ...
          >
            ...
            <span
              className={`absolute left-16 translate-x-2 whitespace-nowrap text-2xl opacity-0 transition-all duration-200 ${pinned ? "translate-x-0 opacity-100" : "group-hover:translate-x-0 group-hover:opacity-100"}`}
            >
              Home
            </span>
          </Link>
      ...
      <div className="border-t-2 flex-col gap-2 py-4 w-full border-dashed flex justify-center items-center border-[#c9a030]">
        <div className="bg-[#c9a030] w-12 h-12 shrink-0 flex justify-center items-center text-center rounded-full overflow-hidden">
          {image ? (
            <Image
              src={image}
            ...
            <div className="translate-y-[4px] select-none text-4xl text-[#2A1A08] font-bold">
        ...
        <div>
          <button
            ...
            className={`text-xl outline-none cursor-pointer text-[#F5E4B0] hover:text-[#AF8937] transition-all duration-300`}
          >
            Logout
          </button>
        </div>
