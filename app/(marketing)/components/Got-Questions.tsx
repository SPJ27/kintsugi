...
export default function GotQuestions() {
    ...
    return (
        <div className="flex min-h-fit sm:min-h-[80vh] flex-col">
            ...
            <div className="flex justify-center transition-all duration-300 items-center flex-col gap-4 mx-4 sm:mx-8">
                {questions.map((question) => {
                    const isOpen = openId === question.id
                    return (
                        <div key={question.id} onClick={() => toggleQuestion(question.id)} className="bg-[#2A1A08] transition-all duration-300 w-full py-4 border-4 border-dashed border-[#c9a030] px-4 sm:px-12 rounded-3xl cursor-pointer hover:-translate-y-[2px]">
                            <div className="flex items-center gap-3 sm:gap-4">
                                <div className="relative w-12 h-9 sm:w-16 sm:h-12 shrink-0">
                            ...
                            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? "max-h-40 opacity-100 mt-2" : "max-h-0 opacity-0 mt-0"}`}>
                                <div className={`text-base sm:text-lg text-[#F5E4B0] font-extralight ${kalam.className} mx-2 sm:mx-18`}>{question.answer}</div>
                            </div>
                        </div>
                    )
                })}
